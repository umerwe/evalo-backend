...
import { buildPaginationMeta, getPaginationParams } from "../../utils/pagination.utils";
import { ApiError } from "../../utils/ApiError";

export const fetchDashboard = async (user: any) => {
    ...
    const stats = {
        total: user?.evaluationStats?.totalAssigned,
        completed: user?.evaluationStats?.totalCompleted,
        pending:
            (user?.evaluationStats?.totalAssigned as number) -
            (user?.evaluationStats?.totalCompleted as number),
        average: user?.evaluationStats?.averageScore,
    };
    ...
    const scores = evaluatedSubmissions.map((e: any) => e.totalScore);
    ...
    scores.forEach((score) => {
        const range = scoreDistribution.find(
...
export const fetchVideos = async (userId: any, query: any) => {
...
export const createEvaluation = async (userId: any, body: any) => {
    const {
        submissionId,
    ...
    if (
        ...
        const avgScore =
            assignment.assignedEvaluators.length > 0
                ? assignment.assignedEvaluators.reduce(
                      (sum, ev) => sum + (ev.totalScore || 0),
                      0
    ...
    const newAverage = (prevAverage * prevTotal + totalScore) / (prevTotal + 1);
...
export const fetchEvaluatedVideos = async (userId: any, query: any) => {
    const { page, limit, skip } = getPaginationParams(query);
